...
### All PTMs
```{r}
dir.create("~/single-cell-histone-ptm/9-differential-abundance/auto+nabut/bulk")
setwd("~/single-cell-histone-ptm/9-differential-abundance/auto+nabut/bulk")
...
# remove unmodified
res.mods <- res[grep("ac|me", rownames(res)), ]
...
pdf("nabut_vs_control_volcano_bulk.pdf",
    height = 5,
    width = 5)
EnhancedVolcano(res.mods,
                lab = res.mods$PTM.Name.Readable,
                title = NULL,
                subtitle = NULL,
                caption = NULL,
                x = 'log2FC',
...
```{r}
dir.create("~/single-cell-histone-ptm/9-differential-abundance/auto+nabut/with-mixed")
setwd("~/single-cell-histone-ptm/9-differential-abundance/auto+nabut/with-mixed")
...
column_ha = HeatmapAnnotation("Batch" = obj.filter.ratio.multi[,obj.filter.ratio.multi$multiplier == 1]$replicate.unique,
...
#### Control Subclustering
```{r}
setwd("~/single-cell-histone-ptm/9-differential-abundance/auto+nabut/with-mixed")

#  subset into just control single cells
obj.filter.ratio.multi.control <- obj.filter.ratio.multi[["ptm.ratio"]][rownames(res.sc[res.sc$p.adj < 0.05,]),obj.filter.ratio.multi$multiplier == 1 & obj.filter.ratio.multi$condition.predict == "control"]

# Scale the data
ptm.ratio.sig.treat <- assay(obj.filter.ratio.multi.control, "MS1.ratio.log1p.batchCorrect")
...
df <- colData(obj.filter.ratio.multi.control) %>%
  as.data.frame() %>%
  rownames_to_column("row_names") %>% # Convert rownames to a column for matching
  mutate(
    control.subcluster = case_when(row_names %in%  names(col_clusters)[col_clusters == "A"] ~ "A", 
                          row_names %in% names(col_clusters)[col_clusters == "B"] ~ "B",
                          TRUE ~ NA_character_))
obj.filter.ratio.multi.control$control.subcluster <- df$control.subcluster
```
...
column_ha = HeatmapAnnotation("Batch" = obj.filter.ratio.multi.control$replicate.unique,
                              "Condition" = obj.filter.ratio.multi.control$condition.predict,
                              "Subcluster" = obj.filter.ratio.multi.control$control.subcluster,
                              col = list("Condition" = c("control" = "#317ec2"),
                                         "Batch" = c("1" = brewer.pal(5, "Set1")[1],
                                                    "2" = brewer.pal(5, "Set1")[2],
                                                    "3" = brewer.pal(5, "Set1")[3],
...
pdf("nabut_vs_control_sig-heatmap_control_subcluster.pdf",
    width = 6,
    height = 8)
Heatmap(scaled_data,
       top_annotation = column_ha,
...
res.sc.control.a <- differential_abundance(obj.filter.ratio.multi.control, 
                                 assay = "MS1.ratio.log1p.batchCorrect",
                                 test_type = "wilcox.test",
                                 group_column = "control.subcluster", 
                                 group1 = "B",
                                 group2 = "A")

res.sc.control.a$PTM.Name.Readable <- rowData(obj.filter.ratio.multi.control)$PTM.Name.Readable[match(rownames(res.sc.control.a), rownames(obj.filter.ratio.multi.control))]

write.csv(res.sc.control.a, "nabut_vs_control_sc_control_subcluster_a.csv")

# remove unmodified
res.sc.control.a.mods <-  res.sc.control.a[grep("ac|me", rownames(res.sc.control.a)), ]

pdf("nabut_vs_control_volcano_control_subcluster_a.pdf",
    height = 6,
    width = 6)
EnhancedVolcano(res.sc.control.a.mods,
                lab = res.sc.control.a.mods$PTM.Name.Readable,
                title = NULL,
                subtitle = NULL,
                caption = NULL,
                x = 'log2FC',
...
column_ha = HeatmapAnnotation(
                              ...
                              Subcluster = obj.filter.ratio.multi.treat$control.subcluster,
...
Heatmap(t(scale(t(subcluster.markers.sig))),
        ...
        cluster_columns = TRUE,
        show_column_names = FALSE,
        column_split = obj.filter.ratio.multi.treat$control.subcluster,  # Use cluster assignments for splitting
        split = histone_proteins,
        cluster_column_slices = FALSE,
        cluster_row_slices = FALSE,
        row_names_gp = gpar(fontsize = 10))
