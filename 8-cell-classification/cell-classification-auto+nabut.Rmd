...
rowData(obj.filter.ratio.multi[["ptm.ratio"]])$Protein.Name.Readable <- protein_readable_translation(rowData(obj.filter.ratio.multi[["ptm.ratio"]])$Protein.Name)
...
## Setup heatmap
```{r}
column_ha = HeatmapAnnotation(Replicate = obj.filter.ratio.multi$replicate.unique,
                              Multiplier = as.factor(obj.filter.ratio.multi$multiplier),
                              ...
                              col = list(Condition = c("control" = "#317ec2",
                                          ...
                                          Multiplier = c("1" = brewer.pal(4, "Paired")[3],
                                                         "100" = brewer.pal(4, "Paired")[4]
                                                      )
                                        )
                              )
```
...
### All PTMs
```{r}
dir.create("/Users/ronaldcutler/Dropbox\ (EinsteinMed)/Sidoli-lab/Projects/Single-cell-histone/Analysis/7-cell-classification/auto+nabut/with_100-cells")
setwd("/Users/ronaldcutler/Dropbox\ (EinsteinMed)/Sidoli-lab/Projects/Single-cell-histone/Analysis/7-cell-classification/auto+nabut/with_100-cells")

# plot ratios
pdf("MS1.ratio.log1p.batchCorrect_all-PTM_heatmap.pdf",
    width = 10,
    height = 10)
Heatmap(t(scale(t(assay(obj.filter.ratio.multi[["ptm.ratio"]], "MS1.ratio.log1p.batchCorrect")), center = FALSE, scale = FALSE)),
        top_annotation = column_ha,
        heatmap_legend_param = list(title = "Normalized Peptide Ratio"),
        cluster_rows = TRUE,
        cluster_columns = TRUE,
        show_column_names = FALSE,
        split = rowData(obj.filter.ratio.multi[["ptm.ratio"]])$Protein.Name.Readable,
...
pdf("MS1.ratio.log1p.batchCorrect_all-PTM_scaled-heatmap.pdf",
    width = 10,
    height = 10)
Heatmap(t(scale(t(assay(obj.filter.ratio.multi[["ptm.ratio"]], "MS1.ratio.log1p.batchCorrect")), center = TRUE, scale = TRUE)),
        top_annotation = column_ha,
...
eigencorplot(p,
             ...
             cexMain = 1.5,
             metavars = c("replicate.unique", "date", "multiplier", "experiment", "condition", "total.ms1"),
             col = viridis(100),
             colCorval = 'firebrick',
             fontCorval = 2,
             cexCorval = 0.5,
...
- This needs to use CV not variance
```{r}
setwd("/Users/ronaldcutler/Dropbox\ (EinsteinMed)/Sidoli-lab/Projects/Single-cell-histone/Analysis/7-cell-classification/auto+nabut/with_100-cells")

# get top 25% variable
top25 <- names(sort(rowVars(assay(obj.filter.ratio.multi[["ptm.ratio"]], "MS1.ratio.log1p.batchCorrect")), decreasing = TRUE))[1:(ceiling(nrow(obj.filter.ratio.multi[["ptm.ratio"]])*0.25))]

df <- assay(obj.filter.ratio.multi[["ptm.ratio"]], "MS1.ratio.log1p.batchCorrect")[rownames(obj.filter.ratio.multi[["ptm.ratio"]]) %in% top25,]
...
# plot ratios
pdf("MS1.global.ratio.batchCorrect_global_heatmap.pdf",
    width = 8,
    height = 3)
Heatmap(t(scale(t(assay(obj.filter.ratio.multi[["global.ratio"]], "MS1.ratio.log1p.batchCorrect")), center = FALSE, scale = FALSE)),
        top_annotation = column_ha,
        heatmap_legend_param = list(title = "Normalized Peptide Ratio"),
        cluster_rows = TRUE,
        cluster_columns = TRUE,
...
# plot Z-score
pdf("MS1.global.ratio.batchCorrect_global_scaled-heatmap.pdf",
    width = 8,
    height = 3)
Heatmap(t(scale(t(assay(obj.filter.ratio.multi[["global.ratio"]], "MS1.ratio.log1p.batchCorrect")), center = TRUE, scale = TRUE)),
        top_annotation = column_ha,
        heatmap_legend_param = list(title = "Z-score"),
        cluster_rows = TRUE,
        cluster_columns = TRUE,
...
# plot ratios
rownames(obj.filter.ratio.multi[["h4.ac.ratio"]]) <- c("0-ac", "Mono-ac", "Di-ac", "Tri-ac", "Tetra-ac")

pdf("MS1.h4.ac.ratio.batchCorrect_heatmap.pdf",
    width = 8,
    height = 3)
Heatmap(t(scale(t(assay(obj.filter.ratio.multi[["h4.ac.ratio"]], "MS1.ratio.log1p.batchCorrect")), center = FALSE, scale = FALSE)),
        top_annotation = column_ha,
        heatmap_legend_param = list(title = "Normalized Peptide Ratio"),
        cluster_rows = FALSE,
        cluster_columns = TRUE,
...
pdf("MS1.h4.ac.ratio.batchCorrect_scaled-heatmap.pdf",
    width = 8,
    height = 3)
Heatmap(t(scale(t(assay(obj.filter.ratio.multi[["h4.ac.ratio"]], "MS1.ratio.log1p.batchCorrect")), center = TRUE, scale = TRUE)),
        top_annotation = column_ha,
        heatmap_legend_param = list(title = "Z-score"),
        cluster_rows = FALSE,
        cluster_columns = TRUE,
...
# PCA
p <- pca(assay(obj.filter.ratio.multi[["h4.ac.ratio"]], "MS1.ratio.log1p.batchCorrect"),
         metadata = colData(obj.filter.ratio.multi),
         center = TRUE, 
         scale = TRUE,
         removeVar = 0)

pdf("h4.ac.ratio.log1p.batchCorrect_all-PTM_eigencor.pdf",
    width = 5,
    height = 5)
eigencorplot(p,
...
pdf("h4.ac.ratio.log1p.batchCorrect_all-PTM_pca.pdf",
    width = 4,
    height = 5)
biplot(p,
       colby = "condition", 
...
- Using bulk samples may distort classification as they were found to cluster differently
...
control_names <- rownames(colData(obj.filter.ratio.multi)[obj.filter.ratio.multi$condition == "control",])
treat_names <- rownames(colData(obj.filter.ratio.multi)[obj.filter.ratio.multi$condition == "treat",])

# Calculate the size of 50% of the vector
size_50_percent_control <- round(length(control_names) * 0.5)
size_50_percent_treat <- round(length(treat_names) * 0.5)

# Randomly select 50% of the names for training
set.seed(123)
...
# Assign the remaining 25% of the names for testing
...
train_condition <- factor(colData(obj.filter.ratio.multi)[colnames(obj.filter.ratio.multi[["h4.ac.ratio"]]) %in% c(control_names_training, treat_names_training),]$condition)
test_condtion <- factor(colData(obj.filter.ratio.multi)[colnames(obj.filter.ratio.multi[["h4.ac.ratio"]]) %in% c(control_names_testing, treat_names_testing),]$condition)

# get scaled h4 data
train <- scale(t(assay(obj.filter.ratio.multi[["h4.ac.ratio"]], "MS1.ratio.log1p.batchCorrect")[,c(control_names_training, treat_names_training)]))
test <- scale(t(assay(obj.filter.ratio.multi[["h4.ac.ratio"]], "MS1.ratio.log1p.batchCorrect")[,c(control_names_testing, treat_names_testing)]))
...
df <- colData(obj.filter.ratio.multi) %>%
  ...
  mutate(
    knn.training.split = case_when(row_names %in% c(control_names_training, treat_names_training) ~ "train", 
                          ...
                          TRUE ~ NA_character_))
obj.filter.ratio.multi$knn.training.split <- df$knn.training.split

# plot
column_ha = HeatmapAnnotation(Multiplier = as.factor(obj.filter.ratio.multi$multiplier),
                              "KNN Training" = as.factor(obj.filter.ratio.multi$knn.training.split),
                              Condition = obj.filter.ratio.multi$condition,
                               col = list("KNN Training" = c("train" = brewer.pal(3, "Set2")[1],
                                                             "test" = brewer.pal(3, "Set2")[2]),
                                           Multiplier = c("1" = brewer.pal(3, "Paired")[1],
                                                         "100" = brewer.pal(3, "Paired")[2]),
...
train_names <- colnames(obj.filter.ratio.multi[["h4.ac.ratio"]])[obj.filter.ratio.multi$condition != "mix"]
test_names <- colnames(obj.filter.ratio.multi[["h4.ac.ratio"]])[obj.filter.ratio.multi$condition == "mix"]

# Assign labels of each condition
train_labels <- factor(colData(obj.filter.ratio.multi)[colnames(obj.filter.ratio.multi[["h4.ac.ratio"]]) %in% train_names,]$condition)

# get scaled h4 data
train <- scale(t(assay(obj.filter.ratio.multi[["h4.ac.ratio"]], "MS1.ratio.log1p.batchCorrect")[,train_names]))
test <- scale(t(assay(obj.filter.ratio.multi[["h4.ac.ratio"]], "MS1.ratio.log1p.batchCorrect")[,test_names]))
...
df <- colData(obj.filter.ratio.multi) %>%
  ...
  mutate(
    knn.mix.predict = case_when(row_names %in%  names(knn.pred.mix)[knn.pred.mix == "control"] ~ "control", 
                          ...
                          TRUE ~ NA_character_))
obj.filter.ratio.multi$knn.mix.predict <- df$knn.mix.predict

# make new condition column that merges known and predicted labels
condition.temp <- as.character(obj.filter.ratio.multi$condition)
condition.temp[condition.temp == "mix"] <- NA
obj.filter.ratio.multi$condition.predict <- coalesce(condition.temp, obj.filter.ratio.multi$knn.mix.predict)

# heatmap
column_ha = HeatmapAnnotation(Multiplier = as.factor(obj.filter.ratio.multi$multiplier),
                              Replicate = obj.filter.ratio.multi$replicate.unique,
                              "KNN Training" = as.factor(obj.filter.ratio.multi$knn.training.split),
                              "KNN Prediction" = as.factor(obj.filter.ratio.multi$knn.mix.predict),
                              "Known Condition" = obj.filter.ratio.multi$condition,
...
### All PTMs
```{r}
dir.create("/Users/ronaldcutler/Dropbox\ (EinsteinMed)/Sidoli-lab/Projects/Single-cell-histone/Analysis/7-cell-classification/auto+nabut/without_100-cells")
setwd("/Users/ronaldcutler/Dropbox\ (EinsteinMed)/Sidoli-lab/Projects/Single-cell-histone/Analysis/7-cell-classification/auto+nabut/without_100-cells")
...
size_50_percent_control <- round(length(control_names) * 0.5)
size_50_percent_control <- round(length(treat_names) * 0.5)

# Randomly select 75% of the names for training
set.seed(123)
control_names_training <- sample(control_names, size = size_50_percent_control)
treat_names_training <- sample(treat_names, size = size_50_percent_treat)
...
# Integrate KNN without 100 cell results into full object
```{r}
df <- colData(obj.filter.ratio.multi) %>%
  as.data.frame() %>%
  rownames_to_column("row_names") %>% # Convert rownames to a column for matching
